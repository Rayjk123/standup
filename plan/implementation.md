...
| 7 — Knowledge bootstrap | 🔨 Steps 0–3 done | Planned in full — see [phase-7.md](phase-7.md). **Step 0** closed the ranking gap (12/12 with embeddings, 11/12 text-only, multi-hop 4/4 — up from a true baseline of 7/8). **Steps 1–3** built and verified: draft storage, the gated `propose_knowledge` tool, and the bootstrap launch route. 39/39 tests. **Step 4 (the research prompt) is written but unproven** — no agent has run it, and a prompt that reads well is precisely this step's failure mode. Next action is one real bootstrap run against this repo, then reading the six docs. 47/47 tests. Steps 5–7 not started. |
